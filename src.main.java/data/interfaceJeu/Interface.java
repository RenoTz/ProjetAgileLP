...
public class Interface extends JFrame {
	...
	public static void creerLaFenetre() {
		...
		panelAdversaire
				.setLayout(new GridLayout(TAILLE_PLATEAU, TAILLE_PLATEAU));
		...
		boutonChangementJoueur.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (joueur.isEnTrainDeJouer()) {
					eventChangementDeJoueur(adversaire, plateauJoueur, false,
							true, true, false);
				} else {
					eventChangementDeJoueur(joueur, plateauAdversaire, true,
							false, false, true);
				}
			}

			private void eventChangementDeJoueur(Joueur joueurProchain,
					Plateau plateau, boolean joueurJoue,
					boolean adversaireJoue, boolean plateauJoueurVisible,
					boolean plateauAdversaireVisible) {
				...
				for (int i = 0; i < plateau.getLePlateau().length; i++) {
					for (int j = 0; j < plateau.getLePlateau().length; j++) {
						if (!plateau.getLePlateau()[i][j].isCaseTouche()) {
							plateau.getLePlateau()[i][j].getBouton()
									.setEnabled(true);
						}
					}
				}
		...
		ajouterLaListeBoutonsAuPanel(panelCoordLettres,
				listeBoutonCoordsLettres);
		ajouterLaListeBoutonsAuPanel(panelCoordChiffres1,
				listeBoutonCoordsChiffres);
		ajouterLaListeBoutonsAuPanel(panelCoordChiffres2,
				listeBoutonCoordsChiffres2);
		...
		for (int i = 0; i < plateau.getLePlateau().length; i++) {
			for (int j = 0; j < plateau.getLePlateau().length; j++) {
				...
				boutonCasePlateau.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						...
						if (!Partie.isStart()) {
							if (premierClic) {
								...
								if (joueur.isEnTrainDeJouer()) {
									rafraichirLaZoneDeJeu(
											joueur.getListeBateaux(),
											plateauJoueur, x, y);
								} else {
									rafraichirLaZoneDeJeu(
											adversaire.getListeBateaux(),
											plateauAdversaire, x, y);
								}
								premierClic = true;
							}
							if (joueur.isTousLesBateauxPlaces()
									&& adversaire.isTousLesBateauxPlaces()) {
							...
							if (joueur.isEnTrainDeJouer()) {
								actionsJoueurs.tirer(joueur, adversaire,
										plateauAdversaire,
										getXPos(plateauAdversaire, x, y),
										getYPos(plateauAdversaire, x, y) - 1,
										boutonScore);
								desactiverToutesLesCasesDuPlateau(plateauAdversaire);
							} else {
								actionsJoueurs.tirer(adversaire, joueur,
										plateauJoueur,
										getXPos(plateauJoueur, x, y),
										getYPos(plateauJoueur, x, y) - 1,
										boutonScore);
							...
							if (aucunJoueurAGagne()) {
								...
								boutonChangementJoueur.setBackground(new Color(
										0, 150, 0));
					...
					private void rafraichirLaZoneDeJeu(
							List<Bateau> listeBateau, Plateau plateau,
							final int x, final int y) {
						afficherLabelProchainBateauAPlacer(listeBateau);
						plateau.getLePlateau()[x][y].getBouton().setEnabled(
								false);
						reactiverLesCasesDuPlateau(plateau, true);
					}

					private void actionPremierClicPlacementBateau(final int x,
							final int y) {
						if (joueur.isEnTrainDeJouer()) {
							panelAdversaire.setVisible(false);
							panelJoueur.setVisible(true);
							desactiverToutesLesCasesDuPlateau(plateauJoueur);
							actionsBateau.saisieDesCoordonneesDesBateaux(
									joueur,
									plateauJoueur,
									FactoryUtils.getXPos(plateauJoueur, x, y),
							...
							desactiverToutesLesCasesDuPlateau(plateauAdversaire);
							actionsBateau.saisieDesCoordonneesDesBateaux(
									adversaire, plateauAdversaire, FactoryUtils
											.getXPos(plateauAdversaire, x, y),
									FactoryUtils.getYPos(plateauAdversaire, x,
											y) - 1);
						}
					}
					...
					private void actionSecondClicPlacementBateau(
						...
						if (joueur.isEnTrainDeJouer()) {
							for (final Bateau bateau : joueur.getListeBateaux()) {
								if (!bateau.isPlace()) {
									if (FactoryUtils
											.convertirCharToInt(plateauJoueur
													.getLePlateau()[x][y]
													.getPoint().getxPos()) > FactoryUtils
											.convertirCharToInt(bateau
													.getTabPoints()[0]
													.getxPos())
											|| plateauJoueur.getLePlateau()[x][y]
													.getPoint().getyPos() > bateau
													.getTabPoints()[0]
													.getyPos()) {
										actionsBateau.assignerCoordonneesBateaux(
												joueur,
												bateau.getTypeBateau(),
												new Points(
														plateau.getLePlateau()[x][y]
																.getPoint()
																.getxPos(),
														plateau.getLePlateau()[x][y]
																.getPoint()
																.getyPos()));
										actionsBateau
												.placerLesBateauxSurLePlateau(
														bateau, plateauJoueur);
										break;
									} else {
										final Points coordonneesArriere = bateau
												.getTabPoints()[0];
										bateau.getTabPoints()[0] = plateau
												.getLePlateau()[x][y]
												.getPoint();
										actionsBateau
												.assignerCoordonneesBateaux(
														joueur,
														bateau.getTypeBateau(),
														coordonneesArriere);
										actionsBateau
							...
							for (final Bateau bateau : adversaire
									.getListeBateaux()) {
								if (!bateau.isPlace()) {
									if (FactoryUtils
											.convertirCharToInt(plateauAdversaire
													.getLePlateau()[x][y]
													.getPoint().getxPos()) > FactoryUtils
											.convertirCharToInt(bateau
													.getTabPoints()[0]
													.getxPos())
											|| plateauAdversaire.getLePlateau()[x][y]
													.getPoint().getyPos() > bateau
													.getTabPoints()[0]
													.getyPos()) {
										actionsBateau.assignerCoordonneesBateaux(
					...
					private Integer getYPos(Plateau plateau, final int i,
							final int j) {
						return plateau.getLePlateau()[i][j].getPoint()
								.getyPos();
					}

					private Integer getXPos(Plateau plateau, final int i,
							final int j) {
						return FactoryUtils.convertirCharToInt(plateau
								.getLePlateau()[i][j].getPoint().getxPos());
					}

					private void afficherLabelProchainBateauAPlacer(
							List<Bateau> listeBateaux) {
						for (final Bateau bateau : listeBateaux) {
							if (!bateau.isPlace()) {
								labelConsole.setText("VEUILLEZ PLACER VOTRE "
										+ bateau.getTypeBateau().toString());
								break;
							}
						}
					}
					...
					private void reactiverLesCasesDuPlateau(Plateau plateau,
						...
						for (int i = 0; i < plateau.getLePlateau().length; i++) {
							for (int j = 0; j < plateau.getLePlateau().length; j++) {
								if (plateau.getLePlateau()[i][j].isWater()
										|| Partie.isStart()) {
									plateau.getLePlateau()[i][j].getBouton()
											.setEnabled(actif);
									plateau.getLePlateau()[i][j].getBouton()
					...
					private void desactiverToutesLesCasesDuPlateau(
							Plateau plateau) {
						for (int i = 0; i < plateau.getLePlateau().length; i++) {
							for (int j = 0; j < plateau.getLePlateau().length; j++) {
								plateau.getLePlateau()[i][j].getBouton()
	...
	private static void ajouterLaListeBoutonsAuPanel(JPanel panel,
			List<JButton> listeBouton) {
	...
	private JButton genererUnBoutonContourPlateau(int i, String type) {
		...
		if (type == TYPE_CHIFFRE) {
			...
			bouton.setText(String.valueOf(FactoryUtils
					.convertirIntToChar(i + 1)));
		}
